...
export default function ContactPopup({ config, onClose, lang, textOverrides }: Props) {
      ...
      <div className={`popup-sheet ${closing ? 'closing' : ''}`} style={{ position: 'relative' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="popup-body">
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.82rem', marginBottom: '1rem' }}>
            {currentLang === 'en' ? 'For further inquiries, please feel free to contact any of the family representatives below.' : 'Untuk pertanyaan lanjut, sila hubungi mana-mana individu di bawah.'}
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {config.contacts.map((c, i) => (
              <div key={i} style={{ background: 'var(--color-surface)', borderRadius: '12px', padding: '1rem', border: '1px solid rgba(201,168,76,.15)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
                <div>
                  <p style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--color-text)' }}>{c.name}</p>
                  <p style={{ fontSize: '0.82rem', color: 'var(--color-text-muted)' }}>{c.phone}</p>
                </div>
...
function PhoneIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.62 1h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 8.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>;
}
